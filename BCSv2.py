...
class PhysicsSimulation:
    def __init__(self, params=None):
        ...
        self.params = {
            ...
            'left_wall_enabled': True, # enable left wall
            'right_wall_enabled': True, # enable right wall
        }
        ...
        # Fixed block sizes - block2 slightly larger than block1
        self.block1_width = 40
        self.block2_width = 50
    ...
    def calculate_cm_position(self):
        ...
        x1 = self.params['block1_position'] + self.block1_width / 2  # Use center of block
        x2 = self.params['block2_position'] + self.block2_width / 2  # Use center of block
    ...
    def check_wall_collision(self):
        ...
        if self.params['left_wall_enabled'] and x1 <= 0:
        ...
        if self.params['right_wall_enabled'] and x2 + self.block2_width >= self.width:
    ...
    def draw(self):
        ...
        if self.params['left_wall_enabled']:
            pygame.draw.line(self.screen, self.BLACK, (0, y), (0, y + self.block_height), 3)
        
        if self.params['right_wall_enabled']:
            pygame.draw.line(self.screen, self.BLACK, (self.width - 1, y), (self.width - 1, y + self.block_height), 3)
...
class SimulationApp:
    def __init__(self, root):
        ...
        self.root.geometry("500x650")  # Increased height for new controls
    ...
    def create_parameter_widgets(self):
        ...
        # Wall settings
        ttk.Label(self.params_frame, text="Wall Settings:").grid(row=7, column=0, sticky="w", padx=5, pady=5)
        
        self.left_wall_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(self.params_frame, text="Enable Left Wall", variable=self.left_wall_var).grid(row=7, column=1, padx=5, pady=5, sticky="w")
        
        self.right_wall_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(self.params_frame, text="Enable Right Wall", variable=self.right_wall_var).grid(row=7, column=2, padx=5, pady=5, sticky="w")
        ...
        # Preset scenarios
        ttk.Label(self.params_frame, text="Preset Scenarios:").grid(row=8, column=0, sticky="w", padx=5, pady=5)
        ...
        for i, (text, value) in enumerate(presets):
            ttk.Radiobutton(self.params_frame, text=text, value=value, variable=self.preset_var, command=self.load_preset).grid(
                row=8, column=i+1, padx=5, pady=5, sticky="w"
            )
    ...
    def get_parameters(self):
        ...
        return {
            ...
            'left_wall_enabled': self.left_wall_var.get(),
            'right_wall_enabled': self.right_wall_var.get(),
        }
    ...
    def load_preset(self):
        ...
        if preset == "equal_masses":
            ...
            self.left_wall_var.set(True)
            self.right_wall_var.set(True)
            ...
            self.left_wall_var.set(True)
            self.right_wall_var.set(False)  # Only need left wall for pi approximation
